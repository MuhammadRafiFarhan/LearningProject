...
void printUI(){
	...
	gotoxy(75,11); printf("'v' untuk akar");
	gotoxy(75,12); printf("'i' untuk sin // 'n' untuk derajat");
	gotoxy(75,13); printf("'o' untuk cos // 'q' untuk derajat");
	gotoxy(75,14); printf("'a' untuk tan // 't' untuk derajat");
	gotoxy(75,15); printf("'l' untuk logaritma");
...
void menu_calculator(){
	...
	do{
		...
		if(result >= 1){
	    gotoxy(0,9);printf("Ingin merubahnya ke dalam bentuk biner? (Klik Y untuk Ya): ");
		...
		gotoxy(0, 13); printf("Klik 'Yes' jika ingin memasukkan operasi matematika yang lain (y/n): ");
		fflush(stdin);
		scanf("%c", &redo);		
	}while(redo != 'n');
...
boolean cekOperator(char opr){
    if(opr == '^' || opr == 'v' || opr == '*' || opr == '/' || opr == '+' || opr == '-' || opr == '.'|| opr == 'i'|| opr == 'o'|| opr == 'a'|| opr == 'l'|| opr == 'n'|| opr == 'q'|| opr == 't'){
        return true;
    }else{
        return false;
    }
